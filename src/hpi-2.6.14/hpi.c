...
static inline void dsp_Set_HRESET( DSP* this, bool hreset )
{
    ...
    else
    {
        ...
        //
        if ( hreset )
            AT91_SYS->PIOC_SODR = AT91C_PIO_PC15; // set PC15 i.e. set HRESET = 1
        else
            AT91_SYS->PIOC_CODR = AT91C_PIO_PC15; // clear PC15 i.e. set HRESET = 0
        }
...
static bool hpi_Construct( HPI* this, int hpi_id )
{
    ...
    // 
    AT91_SYS->PIOC_PER = AT91C_PIO_PC15; // enable direct output enable 
    AT91_SYS->PIOC_IFDR = AT91C_PIO_PC15; // enable direct output enable 
    AT91_SYS->PIOC_IDR = AT91C_PIO_PC15; // enable direct output enable 
    AT91_SYS->PIOC_OER = AT91C_PIO_PC15; // enable direct output enable 
    AT91_SYS->PIOC_SODR = AT91C_PIO_PC15; // set PC15 == high, ie. reset DSP or CPLD

    // Configure CS2 ro access DSP as LCD-like interface
    // i.e. ACCS set to 2 == Two cycles less at the beginning and the end of the access.
    // Default is to have 7 wait states, to accomodate CPLD latency.
    // WS settings will be lowered later, if CPLD is not detected.
    //
    AT91_SYS->EBI_SMC2_CSR[2] =
       ( ( 0 << 28 )         // RWHOLD  Read and Write Signal Hold: 0 cycles
       | ( 0 << 24 )         // RWSETUP Read and Write Signal Setup: 0 cycles
       | ( 1 << 16 )         // ACCS    Address Chip Select Setup: 2 cycles
       | ( 0 << 15 )         // DRP     Data Read Protocol: Standard (not early)
       | ( 2 << 13 )         // DBW     Data Bus Width: 8-bit
       | ( 0 << 12 )         // BAT     Byte Access Type: CS connected to two 8-bit wide devices
       | ( 0 <<  8 )         // DFT     Data Float Time: 0 cycles
       | ( 1 <<  7 )         // WSEN    Wait State Enable: Enabled NWS
       | ( 7 <<  0 )         // NWS     Number of Wait States: 7; HDS width = 5xMCK ~ 80ns
       );

    // printk( "CSR NEW VALUE: %08x\n", AT91_SYS->EBI_SMC2_CSR[2] );

    // Remap memory
    // NCS2: connected to DSP
    //
    this->mem_base = ioremap( 0x30000000, 1 << 16 );

#ifndef ARM_USE_TIMER

    // Allocate IRQ (hardwired to IRQ0)
    AT91_SYS->PIOB_PDR = AT91C_PIO_PB29; // disable PB29, use it as IRQ0

    // Set source mode: external, level triggered, low, highest priority
    AT91_SYS->AIC_SMR[ AT91C_ID_IRQ0 ] = AT91C_AIC_PRIOR_HIGHEST;

    this->irq = AT91C_ID_IRQ0;
    ...
    if ( ebi2hpi_cpld )
    {
        ...
        mdelay( 1 );
        AT91_SYS->PIOC_CODR = AT91C_PIO_PC15; // Clear PC15
        }
    else // EBI2HPI-CPLD not found
    {
        ...
        // Reconfigure CCS2: Speed-up access to DSP (we do not have CPLD between)
        //
        AT91_SYS->EBI_SMC2_CSR[2] =
           ( ( 0 << 28 )         // RWHOLD  Read and Write Signal Hold: 0 cycles
           | ( 0 << 24 )         // RWSETUP Read and Write Signal Setup: 0 cycles
           ...
           | ( 0 <<  8 )         // DFT     Data Float Time: 0 cycles
           | ( 1 <<  7 )         // WSEN    Wait State Enable: Enabled NWS
           | ( 5 <<  0 )         // NWS     Number of Wait States: 5; HDS width = 3xMCK ~ 50ns
           );

        // Speed measurements:
        //
        // GE2P: CSR2 = 00014085, Speed = 44.887 Mbps
        // GE8P: CSR2 = 00014087, Speed = 37.751 Mbps
        }
