 ...
 */
	...
	trace(0, T("webs: Listening for HTTP requests at %s:%d (%s)\n"), websIpaddr, port, websHost );
 ...
 */
			...
			if (wp->flags & WEBS_CGI_REQUEST) {
				if (wp->cgiStdinFd == -1) {
					wp->cgiStdinFd = gopen(wp->cgiStdin, O_CREAT | O_WRONLY | O_BINARY,
						0666);
				}
				gwrite(wp->cgiStdinFd, text, nbytes ); 
 ...
 */
			...
			if (wp->flags & WEBS_CGI_REQUEST) {
				...
				gwrite(wp->cgiStdinFd, text, gstrlen(text));
				gwrite(wp->cgiStdinFd, T("\n"), sizeof(char_t));
			} else
#endif
...
void websFree(webs_t wp)
{
	...
	if ( wp->cgiStdinFd >= 0 )
    {
        trace( 0, "websFree: Closing cgiStdinFd %d\n", wp->cgiStdinFd ); 
		gclose( wp->cgiStdinFd );
        wp->cgiStdinFd = -1;
    }
