...
SXsvfTargetData xsvf_targetData[4] =
{
    { 15,  0, 3, 0,  0 },
    { 10,  5, 2, 1, 31 },
    {  5, 10, 1, 2, 30 },
...
int main( int iArgc, char** ppzArgv )
{
    ...
    for ( i = 1; i < iArgc ; ++i )
    {
        ...
        else if ( !_stricmp( ppzArgv[ i ], "-target" ) )
        {
            ++i;
            if ( i >= iArgc )
            {
                printf( "ERROR:  missing <target> parameter for -target option.\n" );
            }
            else
            {
                iTarget    = atoi( ppzArgv[ i ] );
                printf( "Target = %d\n", iTarget );
            }
        }
        else
    ...
    if ( !pzXsvfFileName )
    {
        printf( "USAGE:  playxsvf [-v level] [-target device_number] filename.xsvf\n" );
        printf( "where:  -v level  = verbose, level = 0-4 (default=0)\n" );
        printf( "        -target device_number = target device number (0-3).\n" );
    ...
    else
    {
        ...
        else
        {
            ...
            startClock  = clock();
            iErrorCode  = xsvfExecute( xsvf_targetData[iTarget].iHir,
                                       xsvf_targetData[iTarget].iTir,
                                       xsvf_targetData[iTarget].iHdr,
                                       xsvf_targetData[iTarget].iTdr,
                                       xsvf_targetData[iTarget].iHdrFpga );
            endClock    = clock();
            fclose( in );
