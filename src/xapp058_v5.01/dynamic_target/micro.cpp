...
int main( int iArgc, char** ppzArgv )
{
    ...
    for ( i = 1; i < iArgc ; ++i )
    {
        ...
        else if ( !_stricmp( ppzArgv[ i ], "-hir" ) )
        {
            ++i;
            if ( i >= iArgc )
            {
                printf( "ERROR:  missing <bits> parameter for -hir option.\n" );
            }
            else
            {
                iHir    = atoi( ppzArgv[ i ] );
        ...
        else if ( !_stricmp( ppzArgv[ i ], "-tir" ) )
        {
            ++i;
            if ( i >= iArgc )
            {
                printf( "ERROR:  missing <bits> parameter for -tir option.\n" );
            }
            else
            {
                iTir    = atoi( ppzArgv[ i ] );
        ...
        else if ( !_stricmp( ppzArgv[ i ], "-hdr" ) )
        {
            ++i;
            if ( i >= iArgc )
            {
                printf( "ERROR:  missing <bits> parameter for -hdr option.\n" );
            }
            else
            {
                iHdr    = atoi( ppzArgv[ i ] );
        ...
        else if ( !_stricmp( ppzArgv[ i ], "-tdr" ) )
        {
            ++i;
            if ( i >= iArgc )
            {
                printf( "ERROR:  missing <bits> parameter for -tdr option.\n" );
            }
            else
            {
                iTdr    = atoi( ppzArgv[ i ] );
        ...
        else if ( !_stricmp( ppzArgv[ i ], "-hdrfpga" ) )
        {
            ++i;
            if ( i >= iArgc )
            {
                printf( "ERROR:  missing <bits> parameter for -hdrfpga option.\n" );
            }
            else
            {
                iHdrFpga    = atoi( ppzArgv[ i ] );
                printf( "HDRFPGA = %d\n", iHdrFpga );
            }
        }
        else
    ...
    if ( !pzXsvfFileName )
    {
        printf( "USAGE:  playxsvf [-v level] [-hir bits] [-tir bits]"
                " [-hdr bits] [-tdr bits] [-hdrfpga bits] filename.xsvf\n" );
        printf( "where:  -v level  = verbose, level = 0-4 (default=0)\n" );
        printf( "        -hir bits = # bits to shift before instruction.\n" );
        printf( "        -tir bits = # bits to shift after instruction.\n" );
        printf( "        -hdr bits = # bits to shift before data.\n" );
        printf( "        -tdr bits = # bits to shift after data.\n" );
        printf( "        -hdrfpga bits = # bits to shift before fpga data.\n" );
    ...
    else
    {
        ...
        else
        {
            ...
            startClock  = clock();
            iErrorCode  = xsvfExecute( iHir, iTir, iHdr, iTdr, iHdrFpga );
            endClock    = clock();
            fclose( in );
